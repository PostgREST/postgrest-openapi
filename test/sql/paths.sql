...
-- TODO: uses a reference for request body when the function
select jsonb_pretty(get_openapi_document('{test}')->'paths'->'/products'->'post'->'requestBody'->'$ref');

-- does not use a reference for request body when the function has no parameters
select get_openapi_document('{test}')->'paths'->'/rpc/returns_simple_type'->'post' ? 'requestBody' as value;
